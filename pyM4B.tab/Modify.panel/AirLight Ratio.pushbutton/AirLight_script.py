...
for r in rooms:
	...
	if wndw_area > 0:
		...
		out.append([room_name(r), wndw_area_sqm, value, wndw_area])
	else:
		out.append([room_name(r), wndw_area_sqm, '-', wndw_area])

out_cut = [s[:3] for s in out]
output.print_table(table_data=sorted(out_cut), columns=['ROOM', 'WINDOWS AREA [sqm]', 'AIR-LIGHT RATIO'], title='OVERALL AIR-LIGHT RATIO')
...
if input('\n\nSET ROOMS PARAMETER WITH THE ABOVE VALUE? [Y/N]'):
	...
	with revit.Transaction('Set AirLight ratio - M4B'):
		if form.values['cb1'] and form.values['cb1'] != '---':
			for sub in out:
				rooms[sub[0]].LookupParameter(form.values['cb1']).Set( sub[3] )
	...
	print('Please type "Y" if you want to set the parameters.')
